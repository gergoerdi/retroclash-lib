...
data Addressing dom addr dat (ts :: [Type]) a where
    Lift :: Addressing0 dom addr dat a -> Addressing dom addr dat '[] a
...
runAddressing1 addr = \case
    ...
    Connect handle -> do
        tell $ AddrMap $ DMap.singleton handle $ fanInMaybe addr -- TODO: block later connections
        return (mempty, ())
    Tell0 (handle :=> susp) -> do
        return (DMap.singleton handle susp, ())
...
toRead
    ...
    -> FanIn dom (Maybe dat)
toRead susps conns = mconcat
    [ rd
    | h :=> (Susp0 mk) <- DMap.toList susps
    , let addr = fromMaybe mempty $ DMap.lookup h conns
    , let rd = gated (delay False $ isJust <$> firstIn addr) $ fanIn $ mk (firstIn addr)
    ]
...
readWrite mkComponent = do
    handle <- Lift $ Component typeRep <$> get <* modify succ
    wr <- Lift ask
    Tell1 $ handle :=> Susp1 (\addr -> mkComponent addr wr)
    return handle
  where
    return = Lift . Clash.Prelude.return
    (>>=) = Bind
    (=<<) = flip (>>=)
    m >> n = Bind m (const n)
